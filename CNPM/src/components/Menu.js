...
class Menu extends Component {
        ...
        if (user.username !== '' ){
            result = (
                <li className="nav-item cta">
                    <a className="nav-link"><span>{user.username}</span></a>
                    <a type="button" onClick={this.onLogout} className="nav-link"><span>Đăng xuất</span></a>
                </li>
            )
        }
        else {
            result = (<li className="nav-item cta">
                <a data-toggle="modal" data-target="#myModal" className="nav-link"><span>Login</span></a>
                <a href="contact.html" className="nav-link nav-item cta"><span>Add listing</span></a>
            </li>
            )
        }
        return result;
    ...
    render() {
        ...
        var elmMenus = menus.map((menu, index) => {
            return (
                <MenuLink label={menu.name} to={menu.to} activeOnly={menu.exact} />
            )
        });
        return (
